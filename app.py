...
def handle_update_chart_event(json, methods=['GET', 'POST']):
    print('Evento recebido: ' + str(json))
    message = chart_data_update_request_handler()
...
def table_data_update_request_handler():
    ...
    for company in companies_data:
        time.sleep(60)
        request_global = requests.get(f'https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={company.company_symbol}&apikey=5365CWXFF00O7QKP')
...
def chart_data_update_request_handler():
    chart_content_data = []
